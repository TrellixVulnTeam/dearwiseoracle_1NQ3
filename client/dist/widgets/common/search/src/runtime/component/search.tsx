...
const SearchInput = (props: SearchSettingProps) => {
  ...
  */
  const initGeocodeList = (configItem: NewDatasourceConfigItem) => {
    ...
    const { configId, hint, geocodeURL, outputDataSourceId, label, searchServiceType, singleLineFieldName } = configItem
    const datasourceListItem = serviceList?.[configId] || {}
    const updateItem = {
      hint: hint,
  ...
  */
  ...
  return (
    <div className={`h-100 align-items-center d-flex flex-grow-1 ${className || ''}`} css={STYLE}>
