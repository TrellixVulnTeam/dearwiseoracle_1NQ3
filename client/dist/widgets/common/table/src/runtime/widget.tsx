...
import { AbstractQueriableDataSource, FeatureDataRecordImpl, FeatureLayerQueryParams } from 'jimu-core/data-source'
...
> {
  ...
  onDataSourceCreated = (dataSource: QueriableDataSource): void => {
    ...
    const isDataView = dataSource?.dataViewId && !isSelectionView
    const hasNoSelectionView = !!dataSource.getMainDataSource().getDataView(DATA_VIEW_ID_FOR_NO_SELECTION)
    const dsChangeCreateTable = !this.updatingTable && ((isSelectionView && hasNoSelectionView) || isDataView || appMode === AppMode.Run)
  ...
  getFeatureLayer = (dataSource: QueriableDataSource, dataRecords?: DataRecord[]) => {
    ...
    if (dataRecords?.length > 0) {
      ...
      if (notToLoad) {
        ...
        return ds.load(q, { widgetId: id }).then(async (records) => {
