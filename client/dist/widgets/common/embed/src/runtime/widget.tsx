...
> {
      ...
      this.setState(
        ...
        () => {
          if (config.embedType === EmbedType.Url) {
            const isUrlResolved = this.isUrlResolved(content)
            if (isUrlResolved) {
  ...
  isUrlResolved = (url) => {
    const P_TAG_REGEXP = /\<p((?!\<p).)+\<\/p\>/gmi
    return !P_TAG_REGEXP.test(url)
  }
  ...
  checkUrl (url: string) {
    ...
    if (supUrls.some(supUrl => url.includes(supUrl))) {
      this.setState({ loadErr: false })
      this.loadContent()
      return
    }
  ...
  render () {
    const { isResetting, loadErr, errMessage, resolveErr, content, codeLimitExceeded } = this.state
