...
export class MarkPopper extends React.PureComponent<Props, States> {
  ...
  // Maintaining layer which required to Draw ,#6679
  // (do this from 'componentWillUnmount' to 'componentDidUpdate', because of MarkPopper is not really uninstalled)
  componentDidUpdate (prevProps: Props, prevState: States) {
    // MapPopper: OPEN ==> CLOSE
    if (prevState.isShowDialog && !this.state.isShowDialog) {
      //this._destroyCurrentDrawTool()
      this._destroyEditLayer()
    }
    // MapPopper: CLOSE ==> OPEN
    if ((!prevState.isShowDialog) && this.state.isShowDialog) {
      this._createEditLayer()
    }
  }
  ...
  _createEditLayer () {
    ...
    if (!this.state.editLayer) {
      this.setState({
        editLayer: new this.GraphicsLayer({
          id: 'bmLayer',
          listMode: 'hide',
  ...
  changeLayerWithGraphics = (viewId, graphics) => {
    ...
    this.state.editLayer && this.state.editLayer.removeAll()
    if (graphicsExist) {
      graphics &&
        graphics.forEach(graphic => {
          const tempGraphic = this.Graphic.fromJSON(graphic)
          tempGraphic.layer = this.state.editLayer
          this.state.editLayer.add(tempGraphic)
        })
    }
    return this.state.editLayer
  }
  ...
  onGraphicEditedCallback = (result: {
    ...
    let noChangeCount = 0
    result.graphics.forEach(graphic => {
    ...
    if (noChangeCount !== result.graphics.length) {
      this.setState({
        configUpdate: true,
