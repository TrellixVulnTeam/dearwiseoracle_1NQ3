/** @jsx jsx */
import { React, jsx, css, ImmutableArray, lodash, loadArcGISJSAPIModule, getAppStore } from 'jimu-core'
...
export function GeometryFromDraw (props: Props) {
  ...
  const applyBufferEffect = React.useCallback(async () => {
    ...
    if (geometry.spatialReference?.isGeographic && !geometry.spatialReference.isWGS84) {
      const serviceUrl = getAppStore().getState().portalSelf?.helperServices?.geometry?.url ?? 'https://utility.arcgisonline.com/arcgis/rest/services/Geometry/GeometryServer'
