/** @jsx jsx */
import { React, jsx, AllWidgetProps, ReactResizeDetector, IMState, classNames } from 'jimu-core'
...
export default class Widget extends React.PureComponent<AllWidgetProps<IMConfig> & Props, State> {
  ...
  async componentDidMount () {
    ...
    this.oicList = []
    this.geometryService = new GeometryService({ url: this.props.portalSelf.helperServices.geometry.url })
    if (this.props.token) {
  ...
  drawImageSourcePoints = (points, imageID) => {
    for (let i = 0; i < points.length; i++) {
      this.graphicsLayer.add(new Graphic({
        ...
        symbol: (imageID === points[i].imageID ? this.activeSourcePointSymbol : this.sourcePointSymbol),
  ...
  updateGraphics = (image) => {
    ...
    if (this.mapView.type === '3d') {
      ...
      for (let v = this.graphicsLayer.graphics.length - 1; v >= 0; v--) {
        if (this.graphicsLayer.graphics.getItemAt(v).attributes &&
          this.graphicsLayer.graphics.getItemAt(v).attributes.id === 'oi-polygons' &&
          this.graphicsLayer.graphics.getItemAt(v).attributes.imageID === image.imageID) {
          const graphic = this.graphicsLayer.graphics.getItemAt(v).clone()
          graphic.symbol = this.activeFrustumSymbol
          graphic.visible = this.coverageFlag.currentCoverage
          this.graphicsLayer.remove(this.graphicsLayer.graphics.getItemAt(v))
          this.graphicsLayer.add(graphic)
        } else if (this.graphicsLayer.graphics.getItemAt(v).attributes &&
          this.graphicsLayer.graphics.getItemAt(v).attributes.id === 'oi-polygons' &&
          this.graphicsLayer.graphics.getItemAt(v).attributes.imageID === this.activeImageID) {
          const graphic = this.graphicsLayer.graphics.getItemAt(v).clone()
          ...
          const graphic = this.graphicsLayer.graphics.getItemAt(v).clone()
          currentGeometry = graphic.geometry.clone()
      ...
      for (let v = this.graphicsLayer.graphics.length - 1; v >= 0; v--) {
        if (this.graphicsLayer.graphics.getItemAt(v).attributes &&
          ...
          this.graphicsLayer.remove(this.graphicsLayer.graphics.getItemAt(v))
          this.graphicsLayer.add(graphic)
        } else if (this.graphicsLayer.graphics.getItemAt(v).symbol.style === 'circle' &&
          this.graphicsLayer.graphics.getItemAt(v).attributes.imageID === image.imageID) {
          const graphic = this.graphicsLayer.graphics.getItemAt(v).clone()
          graphic.symbol = this.activeSourcePointSymbol
          this.graphicsLayer.remove(this.graphicsLayer.graphics.getItemAt(v))
      ...
      if (!this.graphicExists) {
        ...
        for (let v = this.graphicsLayer.graphics.length - 1; v >= 0; v--) {
          if (this.graphicsLayer.graphics.getItemAt(v).symbol?.style === 'circle' && this.graphicsLayer.graphics.getItemAt(v).attributes.imageID === this.activeImageID) {
            const g = this.graphicsLayer.graphics.getItemAt(v).geometry.clone()
            this.graphicsLayer.add(new Graphic({
  ...
  toggleImageInView = (bool) => {
    ...
    for (let s = this.graphicsLayer.graphics.items.length - 1; s >= 0; s--) {
