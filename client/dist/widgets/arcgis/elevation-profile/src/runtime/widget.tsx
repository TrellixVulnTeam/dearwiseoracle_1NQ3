...
export default class Widget extends BaseWidget<AllWidgetProps<IMConfig> & ExtraProps, IState> {
    ...
    if (jmv) {
      this.setState({
        ...
        setTimeout(() => {
          ...
          this.state.jimuMapView.view.map.addMany([this.drawingLayer, this.nextPossibleSelectionLayer])
  ...
  */
  ...
  queryIndividualLayer = async (viewLayers, layer: any, selectedGeometry): Promise<any[]> => {
    ...
    const metersPerSRForLayer = this.getMetersPerSR(getLayer)
    ...
    let result = []
    try {
      if (!layer.dataSourceJson.url) {
        await getLayer.queryFeatures(lineLayerQuery).then((results) => {
          if (results.features.length > 0) {
            results.features.forEach((feature) => {
              const eachGeometry = feature.geometry
              // Z value after query are returned in SR of the Layer data, so convert the z values in map sr unit
                  ...
                  if (eachPath.length > 0) {
                    eachPath.forEach((eachPoint) => {
                      if (eachPoint.length > 1) {
                        eachPoint[2] = (eachPoint[2] * metersPerSRForLayer) / metersPerSRForMap
                      }
                    })
                  }
                })
        ...
        await execute.executeQueryJSON(layer.dataSourceJson.url, lineLayerQuery).then((results) => {
          if (results.features.length > 0) {
            results.features.forEach((feature) => {
              // update the z values in map sr unit
              // multiply the value with metersPerSRForLayer will convert z value in meters
              // after that divide the value by metersPerSRForMap will give the value in mapSR unit
              const eachGeometry = feature.geometry as Polyline
              if (eachGeometry?.paths?.length > 0) {
                eachGeometry.paths.forEach((eachPath) => {
                  if (eachPath.length > 0) {
                    eachPath.forEach((eachPoint) => {
  ...
  getMetersPerSR = (layer) => {
    let metersPerSR = 1
    //get Units from VCS if not found get unit from layers source SR
    ...
    } else if (layer.sourceJSON?.sourceSpatialReference?.wkid) {
      metersPerSR = unitUtils.getMetersPerUnitForSR(new SpatialReference({ wkid: layer.sourceJSON.sourceSpatialReference.wkid }))
    } else if (layer.sourceJSON?.sourceSpatialReference?.wkt) {
      metersPerSR = unitUtils.getMetersPerUnitForSR(new SpatialReference({ wkt: layer.sourceJSON.sourceSpatialReference.wkt }))
    } else if (layer.sourceJSON?.extent?.spatialReference?.wkid) {
      metersPerSR = unitUtils.getMetersPerUnitForSR(new SpatialReference({ wkid: layer.sourceJSON?.extent?.spatialReference?.wkid }))
    } else if (layer.sourceJSON?.extent?.spatialReference?.wkt) {
      metersPerSR = unitUtils.getMetersPerUnitForSR(new SpatialReference({ wkt: layer.sourceJSON?.extent?.spatialReference?.wkt }))
    } else {
      metersPerSR = unitUtils.getMetersPerUnitForSR(new SpatialReference(this.mapView.view.spatialReference.toJSON()))
    }
    return metersPerSR
  }
          ...
          if (feature?.geometry?.paths?.length > 0) {
            ...
            if ((firstPoint && geometryEngine.intersects(endPointsGeometry[0], firstPoint)) ||
              ...
              const polylineSymbol = {
                type: 'simple-line',
                color: [252, 252, 3, 0.8],
                width: this.state?.jimuMapView?.view?.type === '3d' ? 8 : 6
              }
              const polylineGraphic = new Graphic({
  ...
  render () {
    ...
    } else { //if map not selected then display the default widget placeholder of Elevation Profile
      this.resetToDefault()
      return (
        <WidgetPlaceholder
